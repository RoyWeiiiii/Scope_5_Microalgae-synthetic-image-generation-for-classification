...
def evaluate(model, test_images, test_labels, save_dir, iter):
    model.eval()
    with torch.no_grad():
        _, data_recon, _ = model(test_images, test_labels)
        x_recon = data_recon
...
def train(args):
    ...
    with torch.autograd.set_detect_anomaly(True):
        for iteration in progress_bar:
            ...
            optimizer.zero_grad()
            vq_loss, data_recon, perplexity = model(x, y)
            ...
            if iteration % save_interval == 0 or iteration == total_iterations:
                ...
                evaluate(model, test_images.to(DEVICE), test_labels.to(DEVICE), saved_image_folder, iteration)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Conditional VQVAE')
    ...
    parser.add_argument('--save_dir', type=str, default=os.path.join('temp', 'conditional_vqvae_train_results'), help='output results location')
    ...
    parser.add_argument('--decay', type=float, default=0.99, help='Exponential Moving Average decay value')
