############# Adopted from Github "Towards Faster and Stabilized GAN Training" https://github.com/odegeasslbc/FastGAN-pytorch/tree/main #####################
...
from models.fastgan.discriminator import Discriminator
from models.fastgan.generator import Generator
from models.fastgan.blocks.helper import weights_init
...
def train(args):
    ...
    csv_save_path = os.path.join(save_dir, 'loss_per_iteration.csv')
    ...
    netG = Generator(ngf=ngf, nz=nz, nc=3, im_size=image_size)
    netG.apply(weights_init)
    netG.to(DEVICE)
    
    # Prepare Discriminator Model
    netD = Discriminator(ndf=ndf, nc=3, im_size=image_size)
    ...
    if checkpoint != 'None':
        ...
        checkpoint_stub = os.path.basename(checkpoint)
        current_iteration = int(checkpoint_stub.split('_')[-1].split('.')[0]) + 1
        del ckpt
    ...
    if os.path.exists(csv_save_path):
        ...
        loss_values = [[item[1], item[2], item[3]] for item in df.to_records()]
    ...
    for iteration in progress_bar:
        ...
        real_images, _ = next(train_dataloader)
        ...
        if iteration % save_interval == 0 or iteration == total_iterations:
            ...
            with torch.no_grad():
                vutils.save_image(netG(fixed_noise)[0].add(1).mul(0.5), os.path.join(saved_image_folder, f'{iteration}.jpg'), nrow=4)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='FastGAN')
    ...
    parser.add_argument('--save_dir', type=str, default=os.path.join('fastgan_results'), help='output results location')
