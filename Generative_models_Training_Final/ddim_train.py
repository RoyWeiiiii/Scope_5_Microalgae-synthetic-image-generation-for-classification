...
def evaluate(model, x_shape, noise_scheduler, iter, save_dir):
    # Prepare X and Y
    ...
    vutils.save_image(x, os.path.join(save_dir, f"iter_{iter}.png"), normalize=True)
...
def train(args):
    ...
    for iteration in progress_bar:
        ...
        if iteration % save_interval == 0 or iteration == total_iterations:
            # Sample Images
            evaluate(model, x_shape, noise_scheduler, iteration, saved_image_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default=os.path.join('ddim_results'), help='output results location')
